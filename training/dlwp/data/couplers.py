...
class ConstantCoupler():
    ...
    def __init__(
        ...
        if not prepared_coupled_data:  
            print('Assuming coupled data is not preprocessed, averaging fields in as designed in\
 TrailingAverageCoupler. See docs for specifics.')
            self._prepare_coupled_data()
        else: 
            print('**Assuming coupled data has been prepared properly, using coupled field[s] from\
 dataset "as-is"**')
    ...
    def compute_coupled_indices(self, interval, data_time_step):
        ...
        self._coupled_offsets =  np.empty([self.batch_size,
                                          self.coupled_integration_dim,
                                          len(self.input_times)])
        for b in range(self.batch_size):
            for i in range(self.coupled_integration_dim):
                
                self._coupled_offsets[b,i,:] = b + np.array([ts/data_time_step for ts in self.input_times])
    ...
    def set_scaling(self, scaling_da):
        ...
        :param data_time_step: dataset timestep 
        """
        coupled_scaling = scaling_da.sel(index=self.variables).rename({'index':'channel_in'})
        self.coupled_scaling = {'mean':np.expand_dims(coupled_scaling['mean'].to_numpy(),
                                                      (0,2,3,4)),
                                'std':np.expand_dims(coupled_scaling['std'].to_numpy(),
                                                     (0,2,3,4))}
    ...
    def set_coupled_fields(self, coupled_fields):
        ...
        coupled_fields = coupled_fields[:,:,:,self.coupled_channel_indices,:,:].permute(0,2,3,1,4,5)
        self.preset_coupled_fields = th.empty([self.coupled_integration_dim, self.batch_size, self.timevar_dim]+list(self.spatial_dims))
        for i in range(len(self.preset_coupled_fields)):
            self.preset_coupled_fields[i,:,:,:,:,:] = coupled_fields[0,0,-1,:,:,:]
    ...
    def construct_integrated_couplings(self, batch=None, bsize=None,):
        ...
        if self.coupled_mode:
            ...
            self.integrated_couplings = np.empty((bsize,
                                                  self.coupled_integration_dim,
                                                  self.timevar_dim) + 
                                                 self.spatial_dims)
            
            index_range = slice(batch['time'].start,batch["time"].start+self._coupled_offsets[-1,-1,-1]+1)
            ...
            for b in range(bsize):
                for i in range(self.coupled_integration_dim):
                     coupling_temp = \
                        ds_index_range.isel(time=self._coupled_offsets[b,i,:]) 
                     self.integrated_couplings[b,i,:,:,:] = coupling_temp.to_numpy().reshape(
                                                                (self.timevar_dim,) +\
                                                                coupling_temp.shape[2:])

            return self.integrated_couplings.transpose((1,0,2,3,4,5)).astype('float32') # cast to float for compatability 
...
class TrailingAverageCoupler():
 ...
 dataset "as-is"**')
    ...
    def _set_time_increments(self):
        ...
        dt = pd.Timedelta(self.ds.time[1].values - self.ds.time[0].values).total_seconds()
...
({[t.total_seconds() for t in self.input_times]} in secs) are not divisible by dataset dt: {dt}') 
    ...
    def set_coupled_fields(self, coupled_fields):
        ...
        self.preset_coupled_fields = th.concat(averaging_periods, dim=3).permute(0,2,3,1,4,5)
        # flag for construct integrated coupling method to use this array
        self.coupled_mode = True 
    ...
    def construct_integrated_couplings(self, batch=None, bsize=None,):
        ...
        if self.coupled_mode:
            ...
            if (batch is None) or (bsize is None):
                raise ValueError('Coupled fields must be set if no batch or batch size is provided')
            # reset integrated couplings 
            self.integrated_couplings = np.empty((bsize,
                                                  self.coupled_integration_dim,
                                                  self.timevar_dim) + 
            ...
            # extract coupled variables and scale lazily
            ds = (self.ds.inputs.sel(channel_in=self.variables) - self.coupled_scaling['mean']) \
                  / self.coupled_scaling['std']
            # load before entering loop for efficiency 
            ds_index_range = ds.isel(time=index_range).load()
            ...
            return self.integrated_couplings.transpose((1,0,2,3,4,5)).astype('float32') # cast to
                                                                                        # float32 for 
