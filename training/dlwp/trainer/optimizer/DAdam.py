            ...
            for p in group['params']:
                if p.grad is not None:
                    ...
                    if len(state) == 0:
                        ...
                        # Exponential sign
                        state['exp_avg_sgn'] = torch.zeros_like(p, memory_format=torch.preserve_format)
                        state['exp_avg_sgn2'] = torch.zeros_like(p, memory_format=torch.preserve_format)

                    exp_avgs.append(state['exp_avg'])
                    exp_avg_sqs.append(state['exp_avg_sq'])
                    exp_avg_sgns.append(state['exp_avg_sgn'])
...
def adam(params: List[Tensor],
    ...
    if foreach and not torch.jit.is_scripting():
        raise RuntimeError("sgn-Adam-Error")
...
def _single_tensor_adam(params: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        # Dynamic clipping
        temp_bias_correction = 1 - torch.pow(beta1, step_t)
        bound = 2.0 * torch.sqrt(torch.abs(exp_avg) + eps)
        temp_grad = (
                (1 - temp_bias_correction) * grad +
                temp_bias_correction * (torch.clip(grad, min=-bound, max=bound))
        )
        grad = temp_grad
        ...
        if torch.is_complex(param):
            ...
            exp_avg_sgn = torch.view_as_real(exp_avg_sgn)
        ...
        exp_avg_sgn.mul_(qu).add_(torch.sign(grad), alpha=1 - qu)
        ...
        if capturable or differentiable:
            ...
            param.addcdiv_(f * exp_avg, denom, value=-step_size)
